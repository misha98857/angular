 ...
 * @publicApi
 */
export const IMAGE_CONFIG = new InjectionToken<ImageConfig>('ImageConfig', {
  providedIn: 'root',
...
export class NgOptimizedImage implements OnInit, OnChanges, OnDestroy {
  ...
  private getFixedSrcset(): string {
    const finalSrcs = DENSITY_SRCSET_MULTIPLIERS.map(
        (multiplier) => `${this.callImageLoader({
          src: this.ngSrc,
  ...
  private shouldGenerateAutomaticSrcset(): boolean {
    return (
        !this.disableOptimizedSrcset && !this.srcset && this.imageLoader !== noopImageLoader &&
